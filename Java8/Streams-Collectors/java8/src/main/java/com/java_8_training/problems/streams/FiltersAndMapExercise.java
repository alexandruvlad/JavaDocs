...
public class FiltersAndMapExercise {
    ...
    public static  List<Integer> findEvenNumbers() {
        ...
        evenNumbers.addAll(input.filter(i -> i%2==0).collect(Collectors.toList()));
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = personList.stream().filter(n->(n.getName().equals(name))&&(n.getAge().equals(age))).collect(toList());
    ...
    public static List<Integer> doubleNumbers() {
        ...
        doubleNumbers = input.map(i->i*2).collect(toList());
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap(i->i.stream()).collect(toList());
    ...
    public static Optional<Person> findFirstAllMalePersons(List<Person> personList){
        ...
        personOptional = Optional.of(personList.stream().filter(p->p.isMale()).findFirst().get());
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        overAge = personList.stream().allMatch(p->p.getAge()<age);
    ...
    public static int findTheMinimumNumber(){
        Stream<Integer> input =  Stream.of(5, 2, 200, 33, 150, 7);
        //TODO #11
        // HINT: use reduce
        int min =input.reduce(100,(m,a)->(m<a)? m:a);
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = personList.stream().map(a->a.getAge()).reduce(100,(m,a)->(m<a)? m:a);
    ...
    public static double findAgeAverage(List<Person> personList){
        ...
        sum = personList.stream().map(a->a.getAge()).reduce(0,(a,b)->a+b);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = personList.stream().filter(a->a.getName().startsWith("J")).map(a->a.getNationality()).distinct().collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        List<Integer> someNumbers = Arrays.asList(1, 2, 3,81, 4, 5,9);

        //TODO #15
        Optional<Integer> firstSquareDivisibleByThree = Optional.empty();

        firstSquareDivisibleByThree = Optional.of(someNumbers.stream().filter(a->Math.sqrt(a)== (int)Math.sqrt(a) && a%3==0).findFirst().get());
    ...
    public static int sumStringLength(){
        ...
        sum = strings.stream().map(a->a.length()).reduce(0,(acc,l)->acc+l);
    ...
    public static  int sumSalary(List<Person> personList){
        ...
        sum = personList.stream().map(a->a.getSalary()).reduce(0,(acc,b)->acc+b);
